...
# Create a plot of data/time vs energy sub metering, voltage, and reactive power from the UCI dataset
...
# Subset data and create the plots
subset1 <- data[data$Date %in% c("1/2/2007", "2/2/2007"),]
globalPower <- as.numeric(subset1$Global_active_power)
globalReactive <- as.numeric(subset1$Global_reactive_power)
voltage <- as.numeric(subset1$Voltage)
